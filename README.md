...
```
This project was developed during the first semester of 2018 by me and the Ant team,
and although it did not reach more than a TRL 5 model and a paper (of which the abstract 
attached in this post is part and if you need it, feel free to write me an email) the idea
is worth sharing, so I created the following post. 
...
This is how I got to work to analyze if similar systems worked 100% "Cloud Based", and I found surprises such as the Nvidia Shield the one that is capable of streaming a large library of console games with the power of a cell phone chip (Nvidia Tegra 2). Or Latin American platforms such as GLOUD which (although at 720p) managed to stream smooth games and worked with a similar subscription system.
...
First of all I have to clarify that this task is not new at all, it is something that has been carried out within offices for years through a thin client structure (with protocols like PCOIP or RDP) connected to a single server where all the applications and programs runs.
...
Soooo the idea is to make a Personal computer based on a thin client  arq usign a motherboard with low computing power of type: Raspberry, Intel IoT Kit and/or similar, Amoled screen to reduce energy consumption, Lithium battery that generates an autonomy of about 3 hours (In that moment we knew it was little but we left it like that to keep the weight low), Wifi, ethernet (Possibly similar to 3com's XJACK implementation), Bluetooth, GPS, 4G/5G (Depending on contact with operators).
...
the equipment plan. Being a Prepaid user, you must be able to buy credits to add to your account.
...
**Why is this necessary?**: In today's world, the Antbook becomes necessary to respond to the needs of **connectivity** and **portability** since we could carry a very high computing power (even computing power unimagined to easily transport) in low weight.
...
```
    ...
    ~|~(_|\/||(_| |(/__\
        _|         
© Project Developed between Mar. and Jun. 2018, Re-edited on Nov. 2019.
```
